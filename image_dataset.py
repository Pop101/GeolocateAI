...
MEAN = [0.42586421966552734, 0.4108562469482422, 0.38114386796951294]
STD  = [0.287710040807724, 0.2794015109539032, 0.29453545808792114]
...
class ImageDataset(Dataset):
    def __init__(self, image_paths:list[str], output_vals:list[Any], size:tuple=(320, 320), device=torch.device('cpu')):
        self.image_paths = image_paths
        self.output_vals = output_vals
        self.size = size
        self.device = device
        
        assert len(self.image_paths) == len(self.output_vals), "Length of image_paths and output_vals must be the same"
    
    @staticmethod
    def get_transforms(randomize=False) -> transforms.Compose:
        ...
        if randomize:
            return transforms.Compose([
                ...
                ImageDataset.get_transforms(randomize=False)
            ])
        else:
            return transforms.Compose([
    ...
    def __getitem__(self, idx):
        ...
        output_val = self.output_vals[idx]
        
        # Load image. Note it is the user's job to apply transforms if desired
        image = Image.open(image_path).convert("RGB")
        image_tensor = self.get_size_transpose()(image)
        return image_tensor.to(self.device), torch.tensor(output_val, dtype=torch.float32).to(self.device)
...
if __name__ == "__main__":
    # Recalculate stats
    import polars as pl
    df = pl.read_csv("./path_to_coords.csv")
    ds = ImageDataset(df["path"].to_list(), df[["lat", "lon"]].to_numpy().tolist())
    
    print("Calculating stats...")
    
    mean, std = calculate_stats(ds, 1500)
    print("MEAN = ", mean)
    print("STD  = ", std)